...
Function PostReAssignOneProduct() {
[CmdletBinding()]
 Param([parameter(Mandatory = $false)]
		 ...
		 [parameter(Mandatory = $false)]
         [string] $Url = "http://localhost/inf/pda/reassign/oneProduct"
		 )
	...
	$ReAssign.Body.LAYOUT = @{
	'LAYOUT_ID' 			= $LayoutId
	}

	$ReAssign.Body.PRODUCT = @{
	'ITEM_ID' 			= $ItemId
	}
	...
	$JsonRequest = SerializeJSON -Obj $ReAssign
...
foreach ($Tag in $Tags){
	PostReAssignOneProduct -ItemId $Tag.Item -TagId $Tag.Mac -StoreCode $Tag.Store
}